...
-- @Last Modified time: 2024-10-31 23:04:09
local 一键强化符观察 = class()
local tp,zts
local insert = table.insert
-- local qmx = 取普通模型
local qmx = 引擎.取模型
function 一键强化符观察:初始化(根)
    self.ID = 168.1
...
function 一键强化符观察:加载资源()
    self.人物装备 = {}
    local 格子 = tp._物品格子
    local 资源 = tp.资源
    local 底图 =资源:载入('wzife.wdf',"网易WDF动画",0xA393A808)
    for i=1,6 do
        self.人物装备[i] = 格子(0,0,i,"一键强化符观察_人物装备2",底图)
    end
    self.人物装备[1]:置根(tp)
    self.灵饰 = {}
    for i=1,4 do
        self.灵饰[i] = 格子(0,0,i,"一键强化符观察_灵饰2",tp.底图)
    end
    ...
    self.资源组 = {
        ...
        [5] = 按钮(自适应.创建(17,4,80,22,1,3),0,0,4,true,true," 一键强化"),
    }
end
--1650025321 wzife.wd3任务完成
function 一键强化符观察:打开(数据)
    if self.可视 then
        self.可视 = false
        self.资源组=nil
        return
...
function 一键强化符观察:显示(dt,x,y)
    self.焦点 = false
    self.资源组[1]:显示(self.x,self.y)
    self.资源组[2]:更新(x,y)
    self.资源组[5]:更新(x,y)
    self.资源组[5]:显示(self.x+90,self.y+270)
    tp.窗口标题背景_:显示(self.x-86+self.资源组[1].宽度/2,self.y)
    引擎.场景.字体表.标题字体:显示(self.x+self.资源组[1].宽度/2,self.y+3,"一键强化符")
    ...
    zts:显示(self.x+13,self.y+274+24, "附加临时符效果（80w/件）")
    zts:显示(self.x+13,self.y+274+24*2, "下线不会取消临时符效果")
    if self.资源组[5]:是否选中(x,y) and not tp.战斗中 then
         tp.提示:自定义(x+10,y+10,"不穿戴不收费")
    end
...
function 一键强化符观察:检查点(x,y)
    if self.资源组 ~= nil and self.资源组[1]:是否选中(x,y) then
        return true
    end
end

function 一键强化符观察:初始移动(x,y)
    tp.运行时间 = tp.运行时间 + 1
