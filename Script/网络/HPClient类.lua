...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2023-06-26 02:33:25
...
function hpclient:初始化(callback)
	...
	self._sendbuf 	= require("ggebuf")(409600)
	self._recbuf 	= require("ggebuf")(409600)
...
function hpclient:连接(ip,port,async)--地址，端口，异步
	...
	return self._hp:Start(ip or 统一ip,port or 9527,async and 1 or 0) ==1  ----------------客户端ip
	-- return self._hp:Start(ip or '127.0.0.1',port or 9527,async and 1 or 0) ==1  ----------------客户端ip
end
function hpclient:发送(...)
	self.累计发送次数 = self.累计发送次数+1
	self._sendbuf:添加数据(...)
