...
function 场景类_系统商会:初始化(根)
	...
	local 总控件 = self.控件类:创建控件('系统商会总控件')
	总控件:置可视(true,true)
	self.输入框 = 总控件:创建输入("系统商会数量输入",0,0,100,14)
...
function 场景类_系统商会:显示(dt,x,y)
	...
	elseif self.资源组[3]:事件判断() then
		...
		发送数据(80,{类型="购买",数据={数量=self.数量,类型=self.商店类型,页数=self.页数-1,选中=self.选中,环装=self.环装}})
	end
	self.资源组[1]:显示(self.x,self.y)
	tp.窗口标题背景_:显示(self.x-86+self.资源组[1].宽度/2,self.y)
	引擎.场景.字体表.标题字体:显示(self.x+self.资源组[1].宽度/2,self.y+3,"系统商会")
...
function 场景类_系统商会:检查点(x,y)
	if self.资源组[1]:是否选中(x,y)  then
		return true
	end
end

function 场景类_系统商会:初始移动(x,y)
	tp.运行时间 = tp.运行时间 + 1
