...
--======================================================================--
local 场景类_伙伴技能1 = class()
local tp
local ARGB = ARGB

function 场景类_伙伴技能1:初始化(根)
	self.ID = 58
	self.x = 392
	self.y = 105
	self.xx = 0
	...
	local 格子 = 根._技能格子
	for i=1,20 do
		self.技能[i] = 格子(0,0,i,"伙伴技能1")
	end
	for i=1,5 do
		self.大唐技能[i] = 格子(0,0,i,"伙伴技能1")
		self.随机技能[i] = 格子(0,0,i,"伙伴技能1")
	end
	for i=1,20 do
		self.如意神通[i] = 格子(0,0,i,"伙伴技能1")
	end
...
function 场景类_伙伴技能1:打开(技能组,法术类型,人物或宠物,追加法术)--,如意神通
	if self.可视 then
		--战斗类.hbxztx=0
		self.背景 = 1
		self.可视 = false
		...
		if i > 10 then --大于10技能的时候
			self.背景 = 2
			if 全局游戏宽度==1024 then
				self.适配xy={x=0+xadsd,y=-15}
			else
				self.适配xy={x=-97+xadsd,y=-60}
			end
		else
			self.偏移=2
			if 全局游戏宽度==1024 then
				self.适配xy={x=15+xadsd,y=60}
			else
				self.适配xy={x=-97+xadsd,y=0}
			end
		end
		...
		self.窗口时间 = tp.运行时间
		self.可视 = true
		战斗类.伙伴zhil.法术开关=true
	end
end

function 场景类_伙伴技能1:披坚执锐(数据)
	for w=1,5 do
		self.大唐技能[w]:置技能(nil)
		if 数据[w]~=nil then
			local 临时技能=require("script/显示类/技能")()
...
function 场景类_伙伴技能1:更新()
end

function 场景类_伙伴技能1:显示(dt,x,y)
	if not self.可视 then
		return
	end
	...
	if self.披坚执锐显示~=nil then
		tp.技能格子背景_:显示(self.x+10+self.适配xy.x-115,self.y+self.适配xy.y+200)
		self.随机技能[self.可用编号]:置坐标(self.x+10+self.适配xy.x-115,self.y+self.适配xy.y+200) --随机出来的技能
		self.随机技能[self.可用编号]:显示(x,y,true or self.鼠标)
		self.资源组[3]:显示(self.x+10+self.适配xy.x-177,self.y+self.适配xy.y)
		local x1 = 0
		local y1 = 0
		for w=1,5 do
			self.大唐技能[w]:置坐标(self.x+10+self.适配xy.x-147+x1*68,self.y+self.适配xy.y+34+y1*42)
				...
				if w~=self.可选编号 and self.大唐技能[w].技能 then
					...
					if self.大唐技能[w].事件 and self.大唐技能[w].焦点 then--self.鼠标 then
						if tp.选中战斗窗口~=nil and tp.选中战斗窗口~=0 then --测试模式 修改战斗中显示窗口时更改
							...
							战斗类.伙伴zhil:设置披坚执锐(self.大唐技能[w].技能,w)
							self:打开()
						end
					end
				end
	...
	elseif self.如意神通显示~=nil then
		local x2 = 0
		local y2 = 0
		self.资源组[4]:显示(self.x+10+self.适配xy.x-177,self.y+self.适配xy.y)
		for w=1,20 do
			if self.如意神通[w].技能~=nil then
				self.如意神通[w]:置坐标(self.x+10+self.适配xy.x-150+x2*68,self.y+self.适配xy.y+34+y2*42)
				...
				if self.如意神通[w].技能 ~= nil and self.如意神通[w].焦点 then
					...
					if self.如意神通[w].事件 and self.如意神通[w].剩余冷却回合 == nil then  --self.鼠标 没写在下面就不会有鼠标
						if tp.选中战斗窗口~=nil and tp.选中战斗窗口~=0 then --测试模式 修改战斗中显示窗口时更改
							...
							战斗类.伙伴zhil:设置法术参数(self.如意神通[w].技能)
							self:打开()
						end
					end
				end
		...
		tp.字体表.普通字体:置颜色(白色)
		tp.字体表.普通字体:显示(self.x+10+self.适配xy.x-160,self.y+self.适配xy.y+264,"如意神通：")
		tp.字体表.普通字体:显示(self.x+10+self.适配xy.x-91,self.y+self.适配xy.y+264,self.神通技能[1].."，")
		tp.字体表.普通字体:显示(self.x+10+self.适配xy.x-160,self.y+self.适配xy.y+279,self.神通技能[2].."，")
		tp.字体表.普通字体:显示(self.x+10+self.适配xy.x-91,self.y+self.适配xy.y+279,self.神通技能[3].."。")
	end
				...
				if tp.选中战斗窗口~=nil and tp.选中战斗窗口~=0 then --测试模式 修改战斗中显示窗口时更改
					...
					if self.技能[w].事件 and self.鼠标 and self.技能[w].剩余冷却回合 == nil then
						if tp.选中战斗窗口~=nil and tp.选中战斗窗口~=0 then --测试模式 修改战斗中显示窗口时更改
							...
							战斗类.伙伴zhil:设置法术参数(self.技能[w].技能)
							self:打开()
						end
