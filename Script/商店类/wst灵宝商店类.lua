...
-- @Last Modified time: 2024-07-20 17:22:12
...
function 场景类_wst书铁商店类:打开(内容)
    if self.可视 then
        ...
        local 物品格子 = tp._物品格子
        for an=1,25 do
            self.商品[an] = 物品格子(0,an,an,"wst灵宝商店")
        end
        for an=1,25 do
...
function 场景类_wst书铁商店类:显示(dt,x,y)
    ...
    引擎.场景.字体表.标题字体:显示(self.x+self.资源组[1].宽度/2,self.y+3,"免费灵宝")
    self.资源组[3]:更新(x,y)
    self.资源组[3]:显示(self.x+96,self.y+293)
    self.资源组[4]:显示(self.x+9,self.y+28)
    if self.资源组[3]:事件判断() then
        if self.道具==nil or self.选中==0 or self.商品[self.选中]==nil then
            tp.常规提示:打开("#Y/请选择一样道具")
        else
            发送数据(3903.9,{选中=self.选中})
        end
    end
    self:显示_商品(dt,x,y)
end
function 场景类_wst书铁商店类:显示_商品(dt,x,y)
    local xx = 0
    local yy = 0
    for i=1,25 do
