...
-- @Last Modified time: 2024-07-20 17:21:45
local 场景类_wst强化符商店类 = class()
local tp,zts,zts1
...
function 场景类_wst强化符商店类:显示(dt,x,y)
    -- --------------------
    -- 素材位置调整()
    -- ax=AFCHX
    -- ay=AFCHY
    ...
    引擎.场景.字体表.标题字体:显示(self.x+self.资源组[1].宽度/2,self.y+3,"临时符")
    self.资源组[3]:更新(x,y)
    self.资源组[6]:更新(x,y)
    self.资源组[7]:更新(x,y)
...
function 场景类_wst强化符商店类:显示_商品(dt,x,y)
    local xx = 0
    local yy = 0
    for i=1,25 do
...
function 场景类_wst强化符商店类:检查点(x,y)
    if self.资源组[1]:是否选中(x,y)  then
        return true
    end
end

function 场景类_wst强化符商店类:初始移动(x,y)
    tp.运行时间 = tp.运行时间 + 1
...
function 场景类_wst强化符商店类:开始移动(x,y)
    if self.鼠标 then
        self.x = x - self.xx
        self.y = y - self.yy
