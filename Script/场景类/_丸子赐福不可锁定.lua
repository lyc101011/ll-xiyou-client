...
function _丸子赐福:加载资源()
	...
	local 自适应 = tp._自适应
	self.资源组 = {
		[1] = 资源:载入('khzy.dll',"网易WDF动画",0x66100001),--要复制素材文件
		...
		[8] = 资源:载入('khzy.dll',"网易WDF动画",0x66100006),
		[9] = 按钮.创建(资源:载入('khzy.dll',"网易WDF动画",0x66100004),0,0,4,true,true),
		[10] = 资源:载入('khzy.dll',"网易WDF动画",0x66100014), --超级技能2.6 新增
		---------------------超级技能2.19 新增
		[11] = 资源:载入('zdzy.dll',"网易WDF动画","赐福白神柚"),
		[12] = 资源:载入('zdzy.dll',"网易WDF动画","赐福喷射"),
		[13] = 资源:载入('zdzy.dll',"网易WDF动画","赐福技能框单闪"),
	}
	self.锁定按钮组={
...
function _丸子赐福:显示(dt,x,y)
	...
	for n=1,4 do
		self.锁定按钮组[n]:更新(x,y,self.技能[n]~=nil and self.技能[n].可锁定==true)
	...
	elseif self.资源组[6]:事件判断() then
	    ...
	    -------------------------------------
		-- 发送数据(3903,{bb=self.选中bb,认证码=bbs[self.选中bb].认证码,锁定=table.copy(self.锁定),技能=self.丸子赐福.技能}) --超级技能2.6 删
...
function _丸子赐福:锁定判断(bh,bbs)
	...
	if self.丸子赐福.技能[bh]~=nil and qsdcd < 2 and self.锁定[bh]==nil then
		fh=true
	elseif qsdcd >= 2 then
	    fh=false
