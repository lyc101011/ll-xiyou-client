...
function 场景类_商店:刷新MRSPXZ(MRSPXZ)
	self.MRSPXZ={}
	if type(MRSPXZ)=="table" then
		self.MRSPXZ = MRSPXZ --超级技能
	end
end

function 场景类_商店:打开(道具组,名称,类型,银子,MRSPXZ)
	if self.可视 then
		for i=1,20 do
			self.商品[i]:置物品(nil)
		end
		...
		self.银子 = tp.金钱
		if 银子~=nil and tonumber(银子)~=nil then self.银子 = 银子 end
...
function 场景类_商店:显示(dt,x,y)
	...
	elseif self.资源组[3]:事件判断() then
		...
		发送数据(1503,{数量=self.输入框:取文本(),商品=self.道具.原始商品,类型=self.类型})
	end
	self.焦点 = false
	...
	zts:置字间距(15)
	for i=0,3 do
		self.资源组[5]:显示(self.x+101,self.y+249+i*28)
		zts:显示(self.x+50,self.y+253+i*28,sts[i+1])
	end
	zts:置字间距(0)
	...
	for i=1,20 do
		...
		if self.商品[i].物品 ~= nil and self.MRSPXZ[self.商品[i].物品.名称] ~= nil then
			zts1:显示(self.x + xx * 51 + 11,self.y + yy * 51 + 27,self.MRSPXZ[self.商品[i].物品.名称].数量) --超级技能2.5 改
		end
	...
	if self.道具 ~= nil then
		...
		zts:显示(self.x + 78+73-30-12,self.y + 369-60,(self.数量 * self.单价))
	end
	zts:置颜色(tos(self.银子))
	zts:显示(self.x + 78+73-30-12,self.y + 393-57,self.银子)
	self.输入框:置坐标(self.x + 80+73-30-12,self.y + 345-64)
	self.控件类:更新(dt,x,y)
