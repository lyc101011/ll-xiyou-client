...
-- @Last Modified time: 2024-10-29 05:07:30
...
function 场景类_宠物洗练栏:加载资源()
	...
	self.资源组 = {
		...
		[17] = 资源:载入('wzife.wdf',"网易WDF动画",0x661470ed),--认证框
		...
		[23] = 按钮.创建(自适应.创建(12,4,65,22,1,3),0,0,4,true,true," 洗 练 "),
		[24] = 按钮.创建(自适应.创建(12,4,65,22,1,3),0,0,4,true,true," 炼 化 "),
		[25] = 自适应.创建(3,1,70,19,1,3),
...
function 场景类_宠物洗练栏:清除()
 	tp.窗口.召唤兽属性栏:刷新()
	self.材料物品:置物品(nil)
	if self.界面 == 1 and tp.队伍[1].宝宝列表[self.主召唤兽]  ~= nil and self.主召唤兽 ~= 0 then
		local 序列=0
		local bbs = tp.队伍[1].宝宝列表
	    local bb = bbs[self.主召唤兽]
		self.主认证格子:置技能(nil)
		for i=1,24 do
		    self.主技能[i]:置技能(nil)
		end
...
function 场景类_宠物洗练栏:显示(dt,x,y)
	...
	self.资源组[8]:显示(self.x + 208,self.y + 297,nil,nil,nil,self.主状态==1,2)
	self.资源组[9]:显示(self.x + 208,self.y + 342,nil,nil,nil,self.主状态==2,2)
	self.资源组[10]:显示(self.x + 208,self.y + 387)
...
function 场景类_宠物洗练栏:洗练界面(dt,x,y)
	...
	引擎.场景.字体表.标题字体:显示(self.x+self.资源组[1].宽度/2,self.y+3,"洗 炼")
...
function 场景类_宠物洗练栏:炼化界面(dt,x,y)
	...
	ztxx:置颜色(黄色):显示(self.x + 265, self.y + 403,"炼化的召唤兽，炼化后会消失\n炼化可获得天眼珠，三眼天珠或者九眼天珠\n炼化有概率失败。")
	local p = self.开始 - 1
	for h=1,4 do
		for l=1,5 do
			p = p + 1
			...
			if self.物品[p].物品~=nil then
		    	self.物品[p]:显示(dt,x,y,self.鼠标,{0,0,(self.物品[p].物品.总类 == 3 and self.物品[p].物品.分类 == 3) or self.物品[p].物品.总类 == 9527 or self.物品[p].物品.名称 == "吸附石" or self.物品[p].物品.名称 == "圣兽丹"})
		    end
			if self.物品[p].焦点 then
				if self.物品[p].物品 ~= nil then
					tp.提示:道具行囊(x,y,self.物品[p].物品)
					if mouseb(0) and ((self.物品[p].物品.总类 == 3 and self.物品[p].物品.分类 == 3) or self.物品[p].物品.总类 == 9527 or self.物品[p].物品.名称 == "吸附石" or self.物品[p].物品.名称 == "圣兽丹" ) then
				    	if self.材料物品.物品 == nil then
				    		self.材料物品:置物品(self.物品[p].物品)
				    		self.材料物品.物品编号=p
				    		刷新道具逻辑(nil,p,true)
		...
		if bbs[i+self.主加入] ~= nil then
			...
			if bw1:检查点(x,y) then
				...
				if 引擎.鼠标弹起(0) and self.鼠标 and not tp.消息栏焦点 then
					...
					-- for n=1,#bb.内丹 do
					-- 	if bb.内丹[n]~=nil and bb.内丹[n].技能~=nil then
				 --         	local 内丹技能 = 引擎.场景.窗口.召唤兽资质栏:取内丹数据(bb.内丹[n].技能,bb.内丹[n].等级)
				 --         	local 技能 = 引擎.场景.资源:载入(内丹技能.资源,"网易WDF动画",内丹技能.模型)
				 --         	if self.主内丹数据[n] == nil then
				 --         		self.主内丹数据[n] = {}
				 --         	end
				 --         	if self.主内丹数据[n] == nil then
				 --         		self.主内丹数据[n] = {}
				 --         	end
				 --         	self.主内丹数据[n].模型 = 技能
				 --         	self.主内丹数据[n].技能 =  bb.内丹[n].技能
				 --         	self.主内丹数据[n].说明 = 内丹技能.说明
				 --         	self.主内丹数据[n].效果 = 内丹技能.效果
				 --         end
			  --      	end
				end
			end
