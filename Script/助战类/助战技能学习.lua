...
function 助战技能学习:打开(助战)
	if self.可视 then
		...
		self.师门技能 = {}
		self.包含技能 = {}
		self.角色类型=self.助战id.."师门技能"
		if self.角色.门派 == "无门派" then
			tp.常规提示:打开("#Y/加入门派才可以学习师门技能")
			return false
		end
		self:刷新技能(self.角色.师门技能)
		self.选中 = 0
		self.加入 = 0
		tp.运行时间 = tp.运行时间 + 1
		self.窗口时间 = tp.运行时间
...
function 助战技能学习:刷新技能(copyjn)
	self.角色.师门技能={}
	local 临时技能=0
	for k,v in pairs(copyjn) do
		临时技能=require("script/显示类/技能")()
		临时技能:置对象(v.名称,nil,self.角色.门派)
		临时技能.包含技能={}
		临时技能.学会=v.学会
		临时技能.等级=v.等级
		local w = 取包含技能(v.名称,self.角色.奇经八脉["当前流派"])
		for s=1,#w do
			临时技能.包含技能[s] = require("script/显示类/技能")()
			临时技能.包含技能[s]:置对象(w[s],nil,self.角色.门派)
			临时技能.包含技能[s].学会=v.包含技能[s].学会
			临时技能.包含技能[s].等级=v.包含技能[s].等级
		end
		self.角色.师门技能[k]=临时技能
		self.师门技能[k] = zys:载入(临时技能.资源,"网易WDF动画",临时技能.小模型资源)
	end
end
function 助战技能学习:显示(dt,x,y)
	self.资源组[2]:更新(x,y)
	self.资源组[3]:更新(x,y,self.选中 ~= 0 and self.加入 > 0)
	self.资源组[4]:更新(x,y,self.选中 ~= 0 and self.加入 < #self.角色.师门技能[self.选中].包含技能 - 4)
	self.资源组[5]:更新(x,y,false)
	self.资源组[6]:更新(x,y,false)
	self.资源组[7]:更新(x,y,self.选中 ~= 0 and self.角色.师门技能[self.选中].等级 < 180)
	self.焦点 = false
	if self.鼠标 then
		...
		elseif self.资源组[3]:事件判断() or (引擎.取鼠标滚轮() ==1 and self.选中 ~= 0 and self.加入 > 0)  then
			self.加入 = self.加入 - 1
		elseif self.资源组[4]:事件判断() or (引擎.取鼠标滚轮() ==-1 and self.选中 ~= 0 and self.加入 < #self.角色.师门技能[self.选中].包含技能 - 4) then
			self.加入 = self.加入 + 1
		elseif self.资源组[7]:事件判断() then --[7] = 按钮.创建(自适应.创建(12,4,100,22,1,3),0,0,4,true,true,"学 习"),
			 发送数据(2016,{序列=self.选中,助战编号=self.助战编号})
			self.本次需求 = self:学习技能(self.角色.师门技能[self.选中].等级+1)
		end
	end
	-- 显示
	self.资源组[1]:显示(self.x,self.y)
	tp.窗口标题背景_:显示(self.x+110,self.y)
	引擎.场景.字体表.标题字体:显示(self.x+self.资源组[1].宽度/2,self.y+3,self.角色类型)
	...
	if self.角色.门派 ~= "无门派" then
		for n=1,7 do
			...
			if self.选中 ~= n then
				if bw:检查点(x,y) and not tp.消息栏焦点 and self.鼠标 then
					...
					if 引擎.鼠标弹起(0) then
						...
						self.加入 = 0
						self.包含技能 = {}
						if #self.角色.师门技能[self.选中].包含技能 > 0 then
							for i=1,#self.角色.师门技能[self.选中].包含技能 do
								self.包含技能[i] =  zys:载入(self.角色.师门技能[self.选中].包含技能[i].资源,"网易WDF动画",self.角色.师门技能[self.选中].包含技能[i].小模型资源)
							end
						end
						self.介绍文本:清空()
						self.介绍文本:添加文本("#N/【介绍】"..self.角色.师门技能[self.选中].介绍)
						-- self.介绍文本:添加文本("#N/【等级】"..self.角色.师门技能[self.选中].等级)
						self.本次需求 = self:学习技能(self.角色.师门技能[self.选中].等级)
					end
					self.焦点 = true
				end
			else
			...
			if self.角色.额外法宝等级[sm.名称]~=nil then
				fonts:置颜色(红色):显示(self.x + 130+45,self.y + 37 +n*40,"+"..self.角色.额外法宝等级[sm.名称])
				fonts:置颜色(-16777216)
			end
			if self.角色.额外技能等级[sm.名称]~=nil then
				fonts:置颜色(红色):显示(self.x + 130+45,self.y + 37 +n*40,"+"..self.角色.额外技能等级[sm.名称])
				fonts:置颜色(-16777216)
			end
		end
		-- 技能信息类
			...
			if #self.角色.师门技能[self.选中].包含技能 > 0 then
				for i=1,4 do
					if self.包含技能[i] ~= nil then
						...
						if not self.角色.师门技能[self.选中].包含技能[i].学会 then
						   self.包含技能[i+self.加入]:灰度级()
						end
						self.包含技能[i+self.加入]:显示(self.x+212,self.y+i*30+37,缩放2x,缩放2y)

						fonts:显示(self.x+239,self.y+i*30+42,self.角色.师门技能[self.选中].包含技能[i+self.加入].名称)
						if self.角色.师门技能[self.选中].包含技能[i+self.加入].被动~=nil then
						   fonts:显示(self.x+262,self.y+i*30+42,"    （被动技能）")
						end
						if self.包含技能[i+self.加入]:是否选中(x,y) and self.鼠标 then
							tp.提示:技能(x,y,self.角色.师门技能[self.选中].包含技能[i+self.加入],self.角色.师门技能[self.选中].包含技能[i+self.加入].剩余冷却回合)
						end
					end
				end
			end
			fonts:显示(self.x + 85,self.y + 363,self.角色.当前经验)
...
function 助战技能学习:升级技能(技能,需求)
	self.角色.当前经验 = floor(self.角色.当前经验 - 需求.经验/3)
