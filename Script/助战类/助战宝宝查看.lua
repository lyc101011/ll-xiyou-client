...
-- @Last Modified time: 2024-08-14 21:11:17
-- @Author: baidwwy
...
local bb临时潜力=0
local bb未加点潜力=0
local bb临时加点 = {体质=0,魔力=0,力量=0,耐力=0,敏捷=0}
local bb临时属性 = {气血=0,魔法=0,伤害=0,防御=0,速度=0,灵力=0}
local dfhdh=16+16
function 助战宝宝查看:初始化(根)
	self.ID = 178
	self.x = 271
	self.y = 117
	self.xx = 0
	...
	self.确认加点= 按钮.创建(自适应.创建(12,4,70,22,1,3),0,0,4,true,true,"确认加点")
	self.确认加点:绑定窗口_(self.ID)
	self.加点按钮={}
	self.减点按钮={}
	for i=1,5 do
		self.加点按钮[i]= 按钮.创建(自适应.创建(25,4,19,19,4,3),0,0,4,true,true)
		self.减点按钮[i]= 按钮.创建(自适应.创建(26,4,19,19,4,3),0,0,4,true,true)
		...
		if go then
			...
			for i=1,#bb.技能 do
				if jcjn[i]==bb.法术认证 then
					...
					self.技能[num]:置技能(临时技能 , nil , nil , 判断是否赐福技能(bb.赐福技能_生效 ,  bb.技能[i]))
				end
			end
		else
			...
			for i=1,#bb.技能 do
				if jcjn[i].名称==bb.法术认证 then
					...
					local 临时技能=tp._技能.创建()
					临时技能:置对象(jcjn[i],2)
					self.技能[num]:置技能(jcjn[i] , nil , nil , 判断是否赐福技能(bb.赐福技能_生效 ,  bb.技能[i]))
				end
			end
		end
		self.内丹技能 = {}
...
function 助战宝宝查看:刷新宝宝(ssd)
	...
	bb临时潜力=bb.潜力
	bb未加点潜力=bb.潜力
	bb临时加点 = {体质=0,魔力=0,力量=0,耐力=0,敏捷=0}
	bb临时属性 = {气血=0,魔法=0,伤害=0,防御=0,速度=0,灵力=0}
end
...
local bbz= {"气血","魔法","伤害","防御","速度","灵力"}
local bby= {"体质","法力","力量","耐力","敏捷"}
local sxz= {"伤害","防御","速度","灵力"}
local jd3= {"体质","魔力","力量","耐力","敏捷"}
function 助战宝宝查看:显示(dt,x,y)
	self.焦点 = false
	self.资源组[2]:更新(x,y)
	...
	self.资源组[17]:显示(self.x+208+245+dfhdh,self.y+308-62,true,nil,nil,self.状态 == 1,2)
	self.资源组[18]:显示(self.x+208+245+dfhdh,self.y+354-62,true,nil,nil,self.状态 == 2,2)
	self.资源组[19]:显示(self.x+208+245+dfhdh,self.y+400-62,true,nil,nil,self.状态 == 3,2)
	tp.窗口标题背景_:显示(self.x-76+self.资源组[1].宽度/2,self.y)
	引擎.场景.字体表.标题字体:显示(self.x+14+self.资源组[1].宽度/2,self.y+3,"助战宝宝查看")
	...
	for i=1,9 do
		self.资源组[10]:显示(self.x+364+dfhdh,self.y+8+23*i)
	end
	...
	if self.动画.主体 ~= nil then
		tp.影子:显示(self.x + 100+15,self.y+152)
		self.动画.主体:更新()
		self.动画.主体:显示(self.x + 100+15,self.y+152)
		if self.动画.身体 ~= nil then
			self.动画.身体:更新()
			self.动画.身体:显示(self.x + 10+150,self.y+152)
		end
		if self.动画.饰品 ~= nil then
			self.动画.饰品:更新()
			self.动画.饰品:显示(self.x + 100+15,self.y+152)
		end
	end
	...
	if shux=="最大气血" then
		字体:置颜色(0xffCC0033):显示(self.x + 62-5,self.y +241-20+23*1,format("%d/%d",bb.气血,bb.最大气血))
	else
		字体:置颜色(-16777216):显示(self.x + 62-5,self.y +241-20+23*1,format("%d/%d",bb.气血,bb.最大气血))
	end
	if shux=="伤害" then
		字体:置颜色(0xffCC0033):显示(self.x + 62-5,self.y +241-20+23*3,bb.伤害)
	else
		字体:置颜色(-16777216):显示(self.x + 62-5,self.y +241-20+23*3,bb.伤害)
	end
	if shux=="防御" then
		字体:置颜色(0xffCC0033):显示(self.x +  62-5,self.y +241-20+23*4,bb.防御)
	else
		字体:置颜色(-16777216):显示(self.x +  62-5,self.y +241-20+23*4,bb.防御)
	end
	if shux=="速度" then
		字体:置颜色(0xffCC0033):显示(self.x +  62-5,self.y +241-20+23*5,bb.速度)
	else
		字体:置颜色(-16777216):显示(self.x +  62-5,self.y +241-20+23*5,bb.速度)
	end
	if shux=="灵力" then
		字体:置颜色(0xffCC0033):显示(self.x +  62-5,self.y +241-20+23*6,bb.灵力)
	else
		字体:置颜色(-16777216):显示(self.x +  62-5,self.y +241-20+23*6,bb.灵力)
	end
	local wwsx = {体质=bb.体质+bb.特性五维.体质,魔力=bb.魔力+bb.特性五维.魔力,力量=bb.力量+bb.特性五维.力量,耐力=bb.耐力+bb.特性五维.耐力,敏捷=bb.敏捷+bb.特性五维.敏捷}
	字体:置颜色(-16777216)
	字体:显示(self.x + 62-5,self.y +241-20+23*2,format("%d/%d",bb.魔法,bb.最大魔法))
	字体:显示(self.x + 195-5,self.y +241-20+23*1,wwsx.体质)
	字体:显示(self.x + 195-5,self.y +241-20+23*2,wwsx.魔力)
	字体:显示(self.x + 195-5,self.y +241-20+23*3,wwsx.力量)
	字体:显示(self.x + 195-5,self.y +241-20+23*4,wwsx.耐力)
	字体:显示(self.x + 195-5,self.y +241-20+23*5,wwsx.敏捷)
	字体:显示(self.x + 195-5,self.y +241-20+23*6,bb临时潜力)
	字体:显示(self.x + 374+dfhdh,self.y +12+23*1,bb.攻击资质)--+bb.攻击资质+bb.饰品资质.攻击资质
	字体:显示(self.x + 374+dfhdh,self.y +12+23*2,bb.防御资质)--+bb.防御资质+bb.饰品资质.防御资质
	字体:显示(self.x + 374+dfhdh,self.y +12+23*3,bb.体力资质)--+bb.体力资质+bb.饰品资质.体力资质
	字体:显示(self.x + 374+dfhdh,self.y +12+23*4,bb.法力资质)--+bb.法力资质+bb.饰品资质.法力资质
	字体:显示(self.x + 374+dfhdh,self.y +12+23*5,bb.速度资质)--+bb.速度资质+bb.饰品资质.速度资质
	字体:显示(self.x + 374+dfhdh,self.y +12+23*6,bb.躲闪资质)--+bb.躲闪资质+bb.饰品资质.躲闪资质
	if bb.种类=="神兽" then
		字体:显示(self.x + 374+dfhdh,self.y +12+23*7,"★永生★")
	else
		字体:显示(self.x + 374+dfhdh,self.y +12+23*7,bb.寿命)
	end
	字体:显示(self.x + 374+dfhdh,self.y +12+23*8,bb.成长)
	字体:显示(self.x + 374+dfhdh,self.y +12+23*9,bb.五行)
	字体:置颜色(-65536)
	-- local bbz= {"气血","魔法","攻击","防御","速度","灵力"}
	for i=1,#bbz do
		if bb临时属性[bbz[i]]  and  bb临时属性[bbz[i]]>0 then
			字体:显示(self.x+121,self.y+241-20+i*23,"+" ..bb临时属性[bbz[i]])
		end
	end
	if bb临时加点.体质 and  bb临时加点.体质>0 then
		字体:显示(self.x+15+43+140+18+5,self.y+241-20+23*1," +" ..(bb临时加点.体质 or ""))
	end
	if bb临时加点.魔力 and  bb临时加点.魔力>0 then
		字体:显示(self.x+15+43+140+18+5,self.y+241-20+2*23," +" ..(bb临时加点.魔力 or ""))
	end
	if bb临时加点.力量 and  bb临时加点.力量>0 then
		字体:显示(self.x+15+43+140+18+5,self.y+241-20+3*23," +" ..(bb临时加点.力量 or ""))
	end
	if bb临时加点.耐力 and  bb临时加点.耐力>0 then
		字体:显示(self.x+15+43+140+18+5,self.y+241-20+4*23," +" ..(bb临时加点.耐力 or ""))
	end
	if bb临时加点.敏捷 and  bb临时加点.敏捷>0 then
		字体:显示(self.x+15+43+140+18+5,self.y+241-20+5*23," +" ..(bb临时加点.敏捷 or ""))
	end
	字体:置颜色(0xff009933)
	if bb.临时资质 then
		if bb.临时资质.攻击资质>0 then
			字体:显示(self.x+374+35+dfhdh,self.y+12+23*1,"+"..bb.临时资质.攻击资质)
		elseif bb.临时资质.防御资质>0 then
			字体:显示(self.x+374+35+dfhdh,self.y+12+23*2,"+"..bb.临时资质.防御资质)
		elseif bb.临时资质.体力资质>0 then
			字体:显示(self.x+374+35+dfhdh,self.y+12+23*3,"+"..bb.临时资质.体力资质)
		elseif bb.临时资质.法力资质>0 then
			字体:显示(self.x+374+35+dfhdh,self.y+12+23*4,"+"..bb.临时资质.法力资质)
		elseif bb.临时资质.速度资质>0 then
			字体:显示(self.x+374+35+dfhdh,self.y+12+23*5,"+"..bb.临时资质.速度资质)
		elseif bb.临时资质.躲闪资质>0 then
			字体:显示(self.x+374+35+dfhdh,self.y+12+23*6,"+"..bb.临时资质.躲闪资质)
		end
	end
	...
	字体:置颜色(白色)
	字体:置字间距(10)
	字体:显示(self.x + 261+dfhdh,self.y +12+23*1,"攻击资质")
	字体:显示(self.x + 261+dfhdh,self.y +12+23*2,"防御资质")
	字体:显示(self.x + 261+dfhdh,self.y +12+23*3,"体力资质")
	字体:显示(self.x + 261+dfhdh,self.y +12+23*4,"法力资质")
	字体:显示(self.x + 261+dfhdh,self.y +12+23*5,"速度资质")
	字体:显示(self.x + 261+dfhdh,self.y +12+23*6,"躲闪资质")
	字体:置字间距(58)
	字体:置颜色(-1404907)
	字体:显示(self.x + 261+dfhdh,self.y +12+23*7,"寿命")
	字体:显示(self.x + 261+dfhdh,self.y +12+23*8,"成长")
	字体:显示(self.x + 261+dfhdh,self.y +12+23*9,"五行")
	字体:置字间距(0)
	for i=1,5 do
		self.加点按钮[i]:更新(x,y,bb临时潜力>0,1)
		self.减点按钮[i]:更新(x,y,bb临时加点[jd3[i]]>0,1)
		self.加点按钮[i]:显示(self.x+245-11+16,self.y+179+16+25+i*23)
		self.减点按钮[i]:显示(self.x+245-11+16+24,self.y+179+16+25+i*23)
		if self.加点按钮[i]:事件判断() then
			bb临时潜力=bb临时潜力-1
			bb临时加点[jd3[i]]=bb临时加点[jd3[i]]+1
			self:刷新临时信息(bb)
		end
	...
	self.确认加点:更新(x,y,bb临时潜力~=bb未加点潜力,1)
	self.确认加点:显示(self.x+245-23,self.y+179+16+51+6*23)
	if self.确认加点:事件判断() then
		if bb未加点潜力~=0 and bb未加点潜力>bb临时潜力 then
			发送数据(2020,{加点=bb临时加点,认证码=bb.认证码,助战编号=self.助战编号})
			bb临时加点 = {体质=0,魔力=0,力量=0,耐力=0,敏捷=0}
			bb临时属性 = {气血=0,魔法=0,伤害=0,防御=0,速度=0,灵力=0}
		end
	end
	if self.状态 == 1 then
		...
		if self.技能下一页 == false then
			if self.技能数量>12 then
				self.资源组[16]:更新(x,y)
				self.资源组[16]:显示(self.x+436+dfhdh,self.y+354) --下一页
			...
			for n=1,12 do
				self.资源组[11]:显示(self.x+266+dfhdh+(xx*42),self.y+240+11+(yy*42))
				if self.技能[n].技能 ~= nil then
					self.技能[n]:置坐标(self.x+266+dfhdh+(xx*42),self.y+240+11+(yy*42))
				...
				if bb.法术认证 ~= nil and n ==12 then
					...
					if #bb.技能 <=12 then --法术认证也算是技能的
						self.认证格子:置坐标(self.x+266+dfhdh+(xx*42),self.y+240+11+(yy*42))
						self.认证格子:显示(x,y,"技能查看")
						self.资源组[12]:显示(self.x+392+dfhdh,self.y+336)
			...
			self.资源组[15]:更新(x,y)
			self.资源组[15]:显示(self.x+436+dfhdh,self.y+332-19-60)
			...
			if self.技能下一页1 == false then
				for n=13,24 do
					self.资源组[11]:显示(self.x+dfhdh+266+(xx*42),self.y+240+11+(yy*42))
					if self.技能[n].技能 ~= nil then
						self.技能[n]:置坐标(self.x+266+dfhdh+(xx*42),self.y+240+11+(yy*42))
						self.技能[n]:显示(x,y,"技能查看")
						if self.技能[n].焦点 then
					...
					if bb.法术认证 ~= nil and n ==24 then
						...
						-- end
						self.认证格子:置坐标(self.x+dfhdh+266+(xx*42),self.y+240+11+(yy*42))
						self.认证格子:显示(x,y,self.鼠标)
						self.资源组[12]:显示(self.x+dfhdh+392,self.y+336)
						if self.认证格子.焦点 and self.认证格子.技能 ~= nil then
							tp.提示:技能(x,y,self.认证格子.技能)
						end
					end
	...
	tp.宝宝装备背景[1]:显示(self.x+193+dfhdh,self.y+33)
	tp.宝宝装备背景[2]:显示(self.x+193+dfhdh,self.y+33+63)
	tp.宝宝装备背景[3]:显示(self.x+193+dfhdh,self.y+33+63+63)

	for i=1,3 do
		self.物品[i]:置坐标(self.x + 195+dfhdh ,self.y + 33+63*(i-1))
...
function 助战宝宝查看:刷新临时信息(bb)
	local 最大气血 =ceil (bb临时加点["体质"]*bb.成长*6)
	local 最大魔法 = ceil (bb临时加点["魔力"]*bb.成长*3)
	local 伤害 = ceil (bb临时加点["力量"]*bb.成长)
	local 防御 = ceil (bb临时加点["耐力"]*bb.成长*1.33)
	local 速度 = ceil(((bb.速度资质)*bb临时加点["敏捷"]/1000) )
	local 灵力 = ceil (bb临时加点["体质"]*0.3+bb临时加点["耐力"]*0.2+bb临时加点["力量"]*0.4+bb临时加点["魔力"]*0.7)
	bb临时属性 = {气血=最大气血,魔法=最大魔法,伤害=伤害,防御=防御,速度=速度,灵力=灵力}
end
function 助战宝宝查看:检查点(x,y)
	if self.资源组[1]:是否选中(x,y)  then
		return true
	end
end

function 助战宝宝查看:初始移动(x,y)
	tp.运行时间 = tp.运行时间 + 1
